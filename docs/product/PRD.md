...
1. [CVO 点单] CVO 在前厅输入需求："加 GitHub issue 同步"
   ↓
2. [备料 Mise] 指定一只厨师作为本订单负责人（lead cook），由它和 CVO 澄清需求：
      - 单向还是双向？→ CVO: 单向拉取
      - 冲突怎么办？→ CVO: 记日志
...
3. [CVO 审计划] CVO 看到出菜计划（5 道"菜"/阶段）：
      ...
      CVO 可调整顺序、改派厨师（CVO 作为 owner 有调度权），然后批准
   ↓
4. [Fire! 开火] 厨师们开始做：
   ...
   ↓
5. [互审 + 介入] 每道菜做完，厨师间互审：
      - 宪宪（Claude）写完后端OAuth，砚砚（GPT）互审
      - 互审一致 → 自动通过，记录决策依据，进下一道（不叫人）
      - 互审分歧 or 涉及危险操作 → 叫 CVO，带完整上下文（diff + 分歧点）
   ↓
6. [烧焦了 → 重做] 集成测试这道菜烧焦了（同步引擎返回原始时间戳而非 ISO）：
      - CVO 看错误日志，喊 "重做集成测试"（只重做这道，其他菜不动！）
   ↓
7. [最终验收] 所有菜完成，CVO 最终验收（不可省）→ 订单完成
   ↓
