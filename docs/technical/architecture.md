...
| **实时通信** | **ws (WebSocket)** | 出菜板实时更新、agent 输出流式推送 | Clowder |
...
| **状态管理** | **Zustand** | 轻量、TS 友好、出菜板状态机好用 | — |
...
出菜板本质是一个状态机 + 派生视图，Zustand 的 store + selector 模式正好。Redux 太重，Jotai 的 atom 模型在跨组件共享状态时不如 store 直观。
...
┌─────────────────────────────────────────────────────────────┐
│                      Tauri 桌面壳                            │
│   src-tauri/  (Rust，只做窗口/系统集成/文件权限)             │
└──────────────────────────┬──────────────────────────────────┘
                           │ Tauri IPC / WS
        ┌──────────────────┴──────────────────┐
        ▼                                     ▼
┌─────────────────────┐              ┌─────────────────────┐
│   前端 (Front of     │              │   后端 (Back of      │
│   House / 前厅)      │              │   House / 后厨)      │
│                     │              │                     │
│  React + Vite +     │  ◄── WS ──►  │  Fastify + ws +     │
│  Tailwind + Zustand │              │  better-sqlite3 +   │
│                     │              │  Drizzle            │
│  · 出菜板 (pass     │              │                     │
│    board)           │              │  核心子系统：        │
│  · 订单视图          │              │  · Brigade 身份     │
│  · Free Chat        │              │  · Order/FPP 引擎   │
│  · 厨师画像 UI       │              │  · A2A 路由          │
│  · 品菜 (review)    │              │  · Agent 适配器      │
└─────────────────────┘              │  · 事件存储          │
                                     └────────┬────────────┘
...
| **Front of House**（前端） | 出菜板、品菜、交互、状态渲染 | 协调逻辑（那是后端的事） |
...
- **投影**（出菜板视图）→ SQLite 物化视图，可从事件 rebuild
...
```
[CVO 在前厅输入] "加 GitHub 同步"
        │
        ▼ POST /orders
┌──────────────────────────────────────────────────────────┐
│ server: 创建 order，emit order.kickoff 事件              │
│   → FPP projector 更新投影（order=in_progress）           │
│   → WS 推送：出菜板显示新订单                             │
└───────────────────────┬──────────────────────────────────┘
                        ▼ 拉起 lead-cook（如 @yanzi）
┌──────────────────────────────────────────────────────────┐
│ agents: claude-code 适配器 spawn 子进程                   │
│   注入：砚砚身份 + CVO 需求 + "请澄清需求并出计划"        │
│   流式捕获输出 → WS 推送给前厅                            │
└───────────────────────┬──────────────────────────────────┘
                        ▼ lead-cook 输出出菜计划（5 道菜）
[CVO 在前厅审批计划，可编辑]
                        ▼ POST /orders/:id/approve-plan
┌──────────────────────────────────────────────────────────┐
│ server: 计划批准 → 第一道菜 dish 依赖满足                 │
│   → A2A 路由：谁来做？（@yanzi 或 CVO 指派）              │
│   → emit dish.fired 事件                                  │
└───────────────────────┬──────────────────────────────────┘
                        ▼ agent 执行，流式输出
[出菜板实时显示：🟡 firing]
                        ▼ agent 完成
┌──────────────────────────────────────────────────────────┐
│ server: emit dish.passed 事件                             │
│   → FPP: 这道菜 done，下一道依赖满足 → 下一道 ready       │
│   → WS 推送：出菜板显示 🔵 passed，弹出品菜卡              │
└───────────────────────┬──────────────────────────────────┘
                        ▼ CVO 品菜（看 diff）
[CVO 批准 → 下一道 fired；CVO 打回 → dish.retried]
        ...
        ▼ 所有菜 passed
[CVO 最终验收 → order.accepted（必须 CVO）]
```
...
前端用 Zustand store 订阅 WS，出菜板组件从 store 派生视图。agent 的流式输出直接推到对应厨师的消息气泡（streaming）。
...
| **M5 出菜板 UI** | 可视化看板 + 品菜 + Free Chat | V1 可用 |
