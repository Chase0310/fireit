...
describe('agent lifecycle routes', () => {
  it('POST /agents 创建 → 201 + body', async () => {
    ...
    const res = await app.inject({
      method: 'POST',
      url: '/agents',
      payload: { handle: '@zed', name: 'Zed', role: '前端', specialties: ['fe'], restrictions: [], adapterType: 'claude-code', modelFamily: 'claude', roles: ['member'], color: '#000', personality: '干脆' },
    });
  ...
  it('PATCH /agents/:id 改 personality', async () => {
    ...
    const a = await agentService.createAgent({ handle: '@p', name: 'P', role: 'r', specialties: [], restrictions: [], adapterType: 'codex', modelFamily: 'gpt', roles: ['member'] });
    const res = await app.inject({ method: 'PATCH', url: `/agents/${a.agentId}`, payload: { personality: '变了' } });
  ...
  it('DELETE /agents/:id → 204;再 GET /agents 不含它', async () => {
    const { app, agentService } = makeApp();
    const a = await agentService.createAgent({ handle: '@d', name: 'D', role: 'r', specialties: [], restrictions: [], adapterType: 'codex', modelFamily: 'gpt', roles: ['member'] });
  ...
  it('POST /agents/:id/reset-session → 204;resumeId 清空', async () => {
    const { app, agentService } = makeApp();
    const a = await agentService.createAgent({ handle: '@r', name: 'R', role: 'r', specialties: [], restrictions: [], adapterType: 'codex', modelFamily: 'gpt', roles: ['member'] });
  ...
  it('POST /agents/:id/full-reset → 204', async () => {
    const { app, agentService } = makeApp();
    const a = await agentService.createAgent({ handle: '@f', name: 'F', role: 'r', specialties: [], restrictions: [], adapterType: 'codex', modelFamily: 'gpt', roles: ['member'] });
  ...
  it('POST /agents/:id/restart → 204(no-op;下次自然 resume)', async () => {
    const { app, agentService } = makeApp();
    const a = await agentService.createAgent({ handle: '@rs', name: 'RS', role: 'r', specialties: [], restrictions: [], adapterType: 'codex', modelFamily: 'gpt', roles: ['member'] });
